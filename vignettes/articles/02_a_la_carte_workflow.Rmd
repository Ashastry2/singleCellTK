...
By running `runFindMarker()`, SCTK stores a `data.frame` of marker table to the `metadata` slot of the SCE object. The marker table will include all detected markers for each cluster and statistical values associated to each marker. To fetch the table, users can use the command below, and even add filtering parameters.
