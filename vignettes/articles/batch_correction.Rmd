...
| LIGER [@LiuL2020] | `runLIGER()` — *not currently available* | raw counts | `reducedDim` |
...
*Note: `runLIGER()` is listed for completeness but is not currently exported — the
implementation is commented out in `R/runBatchCorrection.R`. Calling it will fail.*
