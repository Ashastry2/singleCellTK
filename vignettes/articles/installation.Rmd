...
pip install scipy numpy astroid six scrublet scanpy bbknn scanorama louvain leidenalg anndata
```
...
If the version number is not 3.20 or higher, you must upgrade Bioconductor to install the toolkit:

```{R biocmI, eval=FALSE}
BiocManager::install()
```

After you install Bioconductor 3.20 or higher, you should be able to install the toolkit. If you still encounter an error, ensure your Bioconductor packages are up to date by running the following command.
